...
- Re-pinned `RWT_MINTS.localnet` + `USDC_MINTS.localnet` to the
  Fornex VPS test-validator's current runtime mints after a
  `scripts/deploy-fornex.sh` validator reset:

    - USDC: `7fU6MdnASj9ueLXBrhBEnXtrwqMFPq5gqSHy44JbtHCo` (was `T5ZgdZfMqjcFFZwK1Wg9YYQFiziCZAXJTinFuLJr2GK`)
    - RWT:  `5UunX3siy52sM3zA6PqTHs3zU4hwHmVCAZqEsbweR4P5` (was `RSuvkud2x4aj23fvhhF234E3BF3KpKeNaFyRfn28oaY`)

  Source of truth: `data/e2e-bootstrap.json::mints`. Generated by
  `scripts/sync-sdk-localnet-mints.sh`.

Non-breaking — constants point at real on-chain mints (matching
the YD `RWT_MINT` pin baked into the deployed .so by the
migrate-mints pipeline); no SDK API or type changes.
...
- `RWT_MINTS.localnet` + `USDC_MINTS.localnet` updated to VPS Fornex
  validator's current runtime mints (`RSuvkud2...` for RWT,
  `T5ZgdZfM...` for USDC). The previous placeholder values caused
  frontend pool-catalogue filtering to miss the master pool after
  Phase 2 validator reset (app.areal.finance → "Real World Token
  doesn't open").

  Going forward, `scripts/deploy-fornex.sh` warns when the SDK and
  the VPS `data/e2e-bootstrap.json` artifact diverge, and
  `scripts/sync-sdk-localnet-mints.sh` provides an operator-driven
  one-shot re-pin + version bump.

- `isPlaceholderRwtMint(RWT_MINTS.localnet)` now returns `false` (the
  localnet RWT mint is a real on-chain mint regenerated by every
  `deploy-fornex.sh` reset and pinned into the YD contract via the
  migrate-mints pipeline). The guard remains `true` for mainnet/devnet
  placeholder bytes — its sole purpose is to block writes against the
  still-unfilled mainnet slot. JSDoc updated to document this choice.

Non-breaking — constants point at real on-chain mints (matching the
runtime YD pin baked into the deployed .so), no SDK API or type changes.

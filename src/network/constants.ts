 ...
 *
 * Mainnet is the canonical Circle USDC. Devnet uses the Areal-bootstrapped
 * test-USDC mint created by `scripts/bootstrap-init.ts` against the Helius
 * devnet RPC (canonical value lives in `data/devnet-addresses.json::mints.usdc`;
 * NOT Circle's faucet USDC, because the bootstrap pipeline mints into the
 * deployer-owned vault and the Circle faucet mint authority is held by Circle).
 * Localnet points at the Fornex-hosted test-validator's runtime USDC test
 * mint — a random keypair regenerated whenever `scripts/deploy-fornex.sh`
 * resets validator state (see Phase 2 spec).
 *
...
export const USDC_MINTS: Record<ClusterName, PublicKey> = {
  ...
  devnet: new PublicKey('E4HJu85ZmTrfBuy9kXQpehYnJdaHKY9oaEZNRCZcW35a'),
 ...
 *
 * - `mainnet` still carries the R20 placeholder bytes
 ...
 * - `devnet` is the bootstrapped on-chain RWT mint created by
 *   `scripts/bootstrap-init.ts` against the Helius devnet RPC. Canonical
 *   value lives in `data/devnet-addresses.json::mints.rwt`. The
 *   yield-distribution program is built with `--features devnet` so its
 *   compiled-in `RWT_MINT` matches this pubkey.
 ...
 *
 * Re-pin localnet via `scripts/sync-sdk-localnet-mints.sh` after each
 * VPS reset; deploy-fornex.sh warns when this constant drifts from
 * `data/e2e-bootstrap.json::mints.rwt_mint`. Devnet is re-pinned by
 * `scripts/sync-sdk-devnet-mints.sh` after a devnet bootstrap.
 */
export const RWT_MINTS: Record<ClusterName, PublicKey> = {
  mainnet: new PublicKey('6YRfYtkZmqWgz8N3MDeqJRc4vSiJ5VGgiMv4ihYzJyY4'),
  devnet: new PublicKey('AFfBWsDEk4iMWcMbLwqXj4yNrgEPNRCdJn5XG1Fw6F77'),
 ...
 *
 * Devnet and localnet no longer carry the placeholder bytes: both have
 * real on-chain RWT mints created by their respective bootstrap pipelines
 * (`scripts/bootstrap-init.ts` for devnet, `scripts/deploy-fornex.sh` for
 * localnet), with the YD contract built using the matching cluster feature
 * flag so its compiled-in `RWT_MINT` matches the bootstrap pubkey. So this
 * guard returns `false` for `RWT_MINTS.devnet` / `RWT_MINTS.localnet` by
 * design — non-mainnet RWT writes are intentional and supported. The
 * guard's sole purpose is to block mainnet writes against the
 * still-unfilled placeholder slot.
 */
